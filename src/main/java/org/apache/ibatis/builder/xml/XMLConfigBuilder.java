...
public class XMLConfigBuilder extends BaseBuilder {
    ...
    private boolean parsed; //标识是否已经被解析过mybatis-config.xml配置文件
    ...
    //负责解析 <settings> 节点，<settings>节点下的配置是MyBatis全局性配置，他们会改变MyBatis的运行时行为
    private Properties settingsAsProperties(XNode context) {
        ...
        //创建Configuration对应的MetaObClass对象
    ...
    private void typeAliasesElement(XNode parent) {
        if (parent != null) {
            for (XNode child : parent.getChildren()) {//处理全部子节点
                if ("package".equals(child.getName())) {//处理全部<package>节点
                    //获取指定的包名
                    String typeAliasPackage = child.getStringAttribute("name");
                    //通过TypeAliasRegistry扫描指定包中的所有类，并解析@Alias注解，完成别名注册
                    configuration.getTypeAliasRegistry().registerAliases(typeAliasPackage);
                } else {//处理<typeAlias>节点
                    String alias = child.getStringAttribute("alias");//获取指定的别名
                    String type = child.getStringAttribute("type");//获取指定别名对应的类型
                    try {
    ...
    private void pluginElement(XNode parent) throws Exception {
        if (parent != null) {
            for (XNode child : parent.getChildren()) { //遍历全部子节点（<plugin>节点）
            ...
            //所有配置的Interceptor对象都是通过Configuration.interceptorChain字段管理，底层是ArrayList现实
        }
    }
    ...
    //解析mybatis-config.xml配置文件中的<properties>节点，形成java.util.Properties对象，之后
    //将该Properties对象设置到XPathPares和Configuration的variables字段中，在后续解析过程中可用于替换占位符
    private void propertiesElement(XNode context) throws Exception {
        if (context != null) {
            //解析 <properties>的子节点（<property>标签）的name和value属性，并记录到Properties中
            Properties defaults = context.getChildrenAsProperties();
            //解析 <properties>的 resource和url属性，这两个属性用于确定properties配置文件的位置
            ...
            //加载 resource 或者 url 属性指定的properties文件
            if (resource != null) {
            ...
            //更新XPathPares和Configuration的variables字段
