...
public class TypeParameterResolver {
    ...
    public static Type resolveFieldType(Field field, Type srcType) {
        Type fieldType = field.getGenericType();//获取字段声明类型
        Class<?> declaringClass = field.getDeclaringClass();//获取字段定义所在类的Class对象
        return resolveType(fieldType, srcType, declaringClass);//调用resolveType进行后续处理
    }
    ...
    /**
     *  选择合适的方法进行解析
     * @param type 字段、方法返回值、方法参数类型
     * @param srcType 字段、方法返回值、方法参数类型的起始位置
     * @param declaringClass 查找字段、方法返回值、方法参数类型所在的类
     * @return
     */
    ...
    private static ParameterizedType resolveParameterizedType(ParameterizedType parameterizedType,
        ...
        //获取参数类型中的原始类型，例如List<String>的原始类型为List
        Class<?> rawType = (Class<?>) parameterizedType.getRawType();

        //获取参数化类型的类型变量或是实际类型列表，例如Map<Integer,String>的实际泛型列表是Integer和String
        Type[] typeArgs = parameterizedType.getActualTypeArguments();

        Type[] args = new Type[typeArgs.length];
        for (int i = 0; i < typeArgs.length; i++) { //解析参数化类型的类型变量或是实际类型列表
