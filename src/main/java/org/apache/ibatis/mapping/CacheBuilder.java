...
public class CacheBuilder {
    private final String id; //Cache对象的唯一标识，一般情况下为映射文件中配置的namespace
    private Class<? extends Cache> implementation;//Cache接口的真正实现类，默认值是PerpetuaCache
    ...
    public Cache build() {
        //如果decorators和implementation字段为空，则为其设置默认值
        setDefaultImplementations();
        //根据implementation指定的类型，通过反射获取参数为String类型的构造方法，并通过该方法创建Cache对象
        Cache cache = newBaseCacheInstance(implementation, id);
        //根据<cache>节点下的配置  初始化cache对象
        setCacheProperties(cache);
        // issue #352, do not apply decorators to custom caches
        //检测cache对象的类型，如果是PerpetualCache类型，则为其添加decorator集合中的装饰器
