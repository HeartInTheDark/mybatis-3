...
/**
 * 每一个<resultMap>节点都会被解析为一个ResultMap对象
...
public class ResultMap {
    ...
    //记录除了<discriminator>节点之外的其他映射关系（即 ResultMap 对象集合）
    private List<ResultMapping> resultMappings;

    //记录了映射关系中带有ID标识的映射关系 例如<id>节点和<constructor> 节点和<idArg>子节点
    private List<ResultMapping> idResultMappings;
    private List<ResultMapping> constructorResultMappings;//记录了映射关系中带有Constructor标志的映射关系
    ...
    private Set<String> mappedProperties;//记录了所有映射关系中涉及的JavaBean属性的集合
    private Discriminator discriminator;//鉴别器，对应<discriminator>节点

    //是否含有嵌套的结果映射，如果某个映射关系中resultMap属性，且不存在resultSet属性，则为true
    private boolean hasNestedResultMaps;
    private boolean hasNestedQueries;//是否含有嵌套查询，如果某个属性的映射存在select属性，则为true
