...
public class UnpooledDataSource implements DataSource {
    ...
    private ClassLoader driverClassLoader; //加载Driver的加载器
    ...
    private synchronized void initializeDriver() throws SQLException {
        if (!registeredDrivers.containsKey(driver)) { //检测驱动是否已经注册
            ...
            try {
                ...
                //注册驱动，DriverProxy是定义在当前类中的内部类，是Driver的代理类
                DriverManager.registerDriver(new DriverProxy(driverInstance));
                //将驱动添加到registeredDrivers集合
    ...
    private void configureConnection(Connection conn) throws SQLException {
        if (autoCommit != null && autoCommit != conn.getAutoCommit()) {
            conn.setAutoCommit(autoCommit); //设置是否自动提交
        }
